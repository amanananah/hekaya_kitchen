...
export function LessonScreen({ step, onBack, onNext, onFinish }: LessonScreenProps) {
  if (step >= 5) {
    return (
      <View style={styles.completeScreen}>
        ...
        <Text style={styles.completeCopy}>
          Your first Luqaimat attempt is ready to send to Grandma Fatima for a voice review.
        </Text>
  ...
  return (
    <View style={styles.screen}>
      <SubscreenHeader title="Grandchild Mode" subtitle={`Luqaimat · Step ${step} of 5`} onBack={onBack} />
      <View style={styles.progressRow}>
        {[1, 2, 3, 4, 5].map((item) => <View key={item} style={[styles.progress, item <= step && styles.progressComplete]} />)}
      </View>
      <View style={styles.checkpoint}>
        <Text style={styles.eyebrow}>GRANDMA'S VISUAL CHECKPOINT</Text>
        <Text style={styles.checkpointTitle}>Let the dough fall in a slow ribbon.</Text>
        <View style={styles.textureWindow}>
          <View style={styles.textureRibbon} />
        </View>
      </View>
      <View style={styles.coachNote}>
        ...
        <Text style={styles.coachCopy}>
          ...
          Your mixture may be slightly thick. Add one teaspoon of warm water, then compare again.
        </Text>
      </View>
      <View style={styles.buttonRow}>
        <SecondaryButton onPress={() => {}} style={styles.flexOne}>Hear Grandma</SecondaryButton>
...
const styles = StyleSheet.create({
  ...
  textureWindow: { flex: 1, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  textureRibbon: { width: 95, height: 160, borderWidth: 20, borderTopWidth: 0, borderColor: '#D9A657', borderBottomLeftRadius: 50, borderBottomRightRadius: 50, transform: [{ skewX: '-8deg' }] },
